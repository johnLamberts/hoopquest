...
const transporter = nodemailer.createTransport(
  sendGridTransport({
    auth: {
      api_key: environmentConfig.SENDGRID_API_KEY,
    },
  })
);
...
export const sendEmail = (userEmail: string) => {
  const emailContent = {
    from: environmentConfig.ADMIN_SENDGRID_EMAIL,
    to: userEmail,
  ...
  transporter.sendMail(emailContent, (err, _info) => {
    if (err) {
...
export default { sendEmailVerificationEmail, sendEmail };
